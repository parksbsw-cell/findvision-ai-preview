...
## 실제 점검
1. 가상 성인 인물 설명을 분석하고 항목별 수정, 원문 근거, 모순 경고를 확인합니다.
2. 빠른 생성을 실행하고 검수 호출 없이 결과와 측정 시간, 다운로드 버튼이 표시되는지 확인합니다.
3. 10초 뒤 하단 다시 생성하기를 눌러 새 결과와 브라우저 완료 횟수 증가를 확인합니다.
4. 정밀 생성을 실행하고 768×1024 결과, 시도 수와 검수 결과를 확인합니다.
5. 원문 보기 등 화면 상태가 변해도 결과와 완료 횟수가 유지되는지 확인합니다.
6. 안전 검사에 막히면 차단 안내를 확인합니다. 자동 우회 재시도는 하지 않습니다.
